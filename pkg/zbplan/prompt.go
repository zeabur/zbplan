...
2. **Select a base image**: First call get_dockerfile_template with the detected language/framework to get a ready-made template — this saves significant time. Then use list_images and list_tags to pin the exact image tags; prefer the latest version — when a template references node:24-alpine, call list_tags for the major (24) and pin to its newest minor/patch (e.g. node:24.9.0-alpine). Prefer dedicated toolchain images over bare language runtimes:
   ...
   - Nuxt / Nitro → call get_dockerfile_template("nuxt"). Check package.json: if the build script invokes 'nuxt generate' → use the nuxt-static template (zeabur/caddy-static runtime, serves .output/public/ with native SPA/MPA fallback and _headers/_redirects support). Otherwise → use the nuxt-server template and set ENV NITRO_PRESET=node-server before the build step (output lands in .output/server/index.mjs). For Bun-based projects: swap the base image to oven/bun, set NITRO_PRESET=bun, and run with 'bun run .output/server/index.mjs'. Always set NITRO_PRESET explicitly for any Nitro-derived framework so the build output matches the runtime image. For nuxt-static (and the generic static template): also read nuxt.config.ts (or .js/.mjs) and check app.baseURL. If it is set to a non-'/' path (e.g. '/2026/'): (a) change the COPY destination from /usr/share/caddy to /usr/share/caddy<baseURL> (strip trailing slash) so the files land at the right URL prefix, and (b) write a /usr/share/caddy/_redirects file with a single line '/  <baseURL>  301' to redirect root visits to the baseURL. Use a heredoc RUN to write _redirects. Do NOT add custom Caddyfile or nginx config — zeabur/caddy-static already handles SPA fallback, _headers, and _redirects natively.
   - Static sites (zeabur/caddy-static reference): serves /usr/share/caddy on :8080; no extra config needed for SPA or MPA.
     SPA/MPA detection: no /404.html in root → SPA mode (missing routes fall back to /index.html at 200); /404.html present → MPA mode (missing routes serve /404.html at 404).
     _redirects (/usr/share/caddy/_redirects): one rule per line — '<source>  <destination>  <status>'; source must start with '/', destination may be a path or absolute URL; '#' lines are comments.
       Example: '/  /2026/  301' or '/old-page  /new-page  302' or '/google  https://google.com  302'
     _headers (/usr/share/caddy/_headers): path on its own line, then indented 'Header-Name: value' lines.
       Example: '/api/*\n  Cache-Control: no-store\n  X-Content-Type-Options: nosniff'
     Asset edge case: any request whose final URL segment ends with a non-html extension (e.g. /api/v1.2, matched by \.[A-Za-z0-9]+$) is treated as an asset and returns 404 without SPA/MPA fallback — avoid dots in the final segment of SPA routes.
